...
统一截面为 2026-08-17。Benchmark 原始表含 148 条记录，元数据表含 9 条模型记录及 79 条逐字段证据，效率表含 9 条模型记录及 27 条逐指标证据。来源包括同一平台横向评测、Benchmark 榜单及厂商官方报告；版本、工具使用、reasoning 配置、agent scaffold 或快照不同的数据不混入同一比较队列。
...
4. 缺失值不等于零。GLM-5.2 在冻结 cohort 中缺少 5 个核心成绩，其中 HLE 为该 cohort 未报告，四个视觉指标因模型为纯文本而不适用；均不插补，因而不进入要求完整矩阵的 TOPSIS 主排名。其官方 HLE 40.5 另存于独立 Z.ai cohort，不跨口径替换。
...
GLM-5.2 在冻结主 cohort 中只有 4/9 项可用成绩（44.4%），标记为“数据不足、暂不排名”。新增官方报告给出 HLE 40.5（with tools 为 54.7），但测试子集与来源不同，故仅作为补充证据，不替换冻结 cohort 的 NA。四项视觉成绩则因 GLM-5.2 为纯文本模型而不适用。这比将缺失值置零或均值填补更符合题目数据约束。
...
完整排名模型中，Kimi K3 成本为 6 美元、综合得分为 0.5933；Claude Fable 5 按当前官方价计算成本为 20 美元、得分为 0.7254。两者构成 Pareto 前沿。GPT-5.5 与 Kimi 同为 6 美元但性能更低；GPT-5.6 Sol 和 Claude Opus 4.8 也被更低成本且更高得分的模型支配。

预算为 6、10、12、16 美元时，规则“预算内综合得分最高”均推荐 Kimi K3；预算达到 20 美元时推荐 Claude Fable 5。GLM-5.2 的标准成本最低，但因能力覆盖不足不能进入该推荐规则，不能把“未评价”误写成“低性能”。
...
得到 (R^2=0.3147)。样本量仅为 5，说明当前数据只能支持“价格与性能存在弱正相关、但价格解释力有限”的描述，不能进行因果推断。
