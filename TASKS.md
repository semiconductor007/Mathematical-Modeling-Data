...
- [x] 固定 9 个核心指标的 benchmark version、test setting 与 source cohort。
- [x] 生成 6 × 9 核心能力矩阵和 54 行可追溯长表，缺失值保持 `NA`。
- [x] 统一模型元数据、价格与效率单位，保留原始精度。
- [x] 分离效率观测值和严格可比值，不兼容配置不进入横向字段。
- [x] 生成只含最终模型的 merged 数据集，不修改 `data/raw/`。
- [x] 输出 Phase 2 质量报告，并通过自动校验与单元测试。

## Phase 3–8 — 建模、稳健性与论文

- [x] 完成描述统计、Pearson/Spearman 相关性、冗余标记和指标复核。
- [x] 完成效益型标准化、CRITIC 客观赋权和完整案例 TOPSIS 排名。
- [x] 分析 Kimi K3 的逐指标优势、短板和领先差距。
- [x] 建立科研长文本、日常对话、代码开发的组合赋权场景模型。
- [x] 建立标准 API 工作负载、工程效率、Pareto 前沿和预算推荐模型。
- [x] 完成熵权法对照、36 组权重扰动、Kendall 稳定性和探索性回归。
- [x] 生成 4 张可复现 SVG 图表和完整建模报告。
- [x] 建立一键流水线、跨阶段结果校验及完整单元测试。
